...
function handleLine(line ,tags) {
  ...
  var momentSearchCursor = moment(fields[0], "DD/MM/YY").hour(0).minute(1);


  
  BalanceEntry.find({'description' : balanceEntry.description, 'amount': balanceEntry.amount,
      realDate: {"$gte": momentSearchCursor.format('YYYY/MM/DD'), "$lt": momentSearchCursor.add('day',1).format('YYYY/MM/DD')}}, function (err, docs) {
