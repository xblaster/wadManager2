...
var createPerson = function() {
    ...
    for (var i = 0; i < chance.integer({min: 0, max: 2}); i++) {
        ...
        titulaire.lastName = titulaireOwn.lastName;
        titulaire.relation = chance.pick(['epouse','epoux','fille','fils'])
        titulaire.birthday = chance.ngBirthday();
        person.content.titulaires.push(titulaire);
    }

    person.content.visites = [];
    for (var i = 0; i < chance.integer({min: 3, max: 13}); i++) {
        var visit   = {};

        visit.type = chance.pick([
               {name: 'phone', color: 'forestgreen'},
            {name: 'envelope', color: '#990046'},
            {name: 'empire', color: '#d9534f'},
            {name: 'user', color: '#009BA9'}
        ]);


        if (visit.type.name == 'empire') {
            visit.title = "Alerte système : "+chance.sentence({words: 3});
            visit.content = chance.sentence({words: 3});
        }    else {
            visit.title = chance.sentence({words: 5});
            visit.content = chance.paragraph();
        }

        visit.date = moment().add('day', -chance.integer({min: 1, max: 300})).toDate();

        visit.status =  chance.pick(["résolu","résolu","résolu","en cours"]);
        person.content.visites.push(visit);
    }

    for (var i = 0; i < chance.integer({min: 1, max: 2}); i++) {
        var visit   = {};
        ...
        visit.title = 'Reminder '+chance.sentence({words: 2});
        visit.date = moment().add('day', chance.integer({min: 1, max: 30})).toDate();
        visit.content = chance.paragraph();
